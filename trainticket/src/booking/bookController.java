    	...
    	else
    	{
        	...
        	while(resultSet.next())
    		{
    			...
    			while(resultSet0.next())
    			{
    				...
    				pst=con.prepareStatement("SELECT sit,cabin FROM bookingticket WHERE date='"+ ndate +"' && trainname='"+ ntrain +"'");
        			ResultSet resultSet1=pst.executeQuery();
        			while(resultSet1.next())
        			{
        				
        				String newsit=resultSet1.getString("sit");
    					String newcoach=resultSet1.getString("cabin");
    					
    					if(newcoach.equals(Coach) && newsit.equals(st))
    					{
        			...
        			while(resultSet1.next())
        			{
    					...
    					else
    					{
    						...
    						A4.setOnAction(e -> {
    				            ...
    				            PreparedStatement pst5;
    		    				try {
									pst5=con.prepareStatement("INSERT INTO admin_booking(Id,sit, coach, date) VALUES ('"+Id+"','"+ st +"','"+ Coach +"','"+ ndate +"') where Id='"+Id+"'");
									pst5.execute();
...
//
    ...
    void Check(ActionEvent event) throws SQLException, ClassNotFoundException {
    	...
    	ResultSet resultSet1;
    	pst2=con.prepareStatement("SELECT Max(Id) FROM bookingticket");
		...
		while(resultSet1.next())
		{
			...
			String iid=resultSet1.getString("Max(Id)");
    		...
    		while(resultSet3.next())
    		{
            	...
            	pst4=con.prepareStatement("SELECT cabin,sit FROM bookingticket WHERE date='"+userdate+"' && trainname='"+usertrain+"'");
        		resultSet4=pst4.executeQuery();
        		while(resultSet4.next())
        		{
        			String cvin=resultSet4.getString("cabin");
        			String sttt=resultSet4.getString("sit");
        			
        			
        			System.out.println(userdate);
        			
        			if(Coach.equals(cvin))
        			{
        				if(sttt.equals(A1))
        				{
